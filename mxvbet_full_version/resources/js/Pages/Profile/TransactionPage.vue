<template>
    <BaseLayout>
        <div class="flex items-center justify-center min-h-screen  sm:max-w-[690px] lg:max-w-[710px] mx-auto p-4">
            ...
            <div v-if="isLoading === false && wallet" class="flex flex-col w-full p-4 rounded">
                <div class="header flex w-full mb-3" style="background-color: var(--navtop-color-dark);">
                ...
                <div v-if="deposits && wallet" class="mt-3">
                    <div class="relative overflow-x-auto">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="(deposit, index) in deposits.date" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
...
export default {
    ...
    methods: {
        scrollUp() {
            window.scrollBy({
                top: -window.innerHeight / 5, // Subir uma "linha" por vez
                behavior: 'smooth'
            });
        },
        scrollDown() {
            window.scrollBy({
                top: window.innerHeight / 5, // Descer uma "linha" por vez
                behavior: 'smooth'
            });
        },
        getWallet: function() {
...
.scroll-btn-down {
    top: 160px; /* Ajuste a posição conforme necessário */
}

.scroll-btn:hover {
    background-color: #32DF12;
}
</style>
