...
class Core
{
    ...
    public static function DiscountBalance(Wallet $wallet, $bet)
    {
        ...
        if ($wallet->balance + $wallet->balance_withdrawal >= $bet) {

            if ($wallet->balance >= $wallet->balance_withdrawal) {
                $sobra = $bet - $wallet->balance_withdrawal;
    
                $wallet->balance = $wallet->balance - $sobra;
            ...
            if ($wallet->balance <= $wallet->balance_withdrawal) {
                $sobra = $bet - $wallet->balance;
    
                $wallet->balance_withdrawal = $wallet->balance_withdrawal - $sobra;
                $wallet->update(['balance_withdrawal' => $wallet->balance_withdrawal]);
        ...
        if ($wallet->balance > 0 && $wallet->balance < $bet && $wallet->balance_bonus + $wallet->balance  >= $bet) {
        ...
        if ($wallet->balance_withdrawal > 0 && $wallet->balance_withdrawal < $bet && $wallet->balance_bonus + $wallet->balance_withdrawal >= $bet) {
        ...
        if ($wallet->balance_withdrawal + $wallet->balance + $wallet->balance_bonus >= $bet) {

            $restante = $bet - ($wallet->balance_withdrawal + $wallet->balance);
            $wallet->decrement('balance_bonus', $restante);
            $wallet->update(['balance' => 0]);
            $wallet->update(['balance_withdrawal' => 0]);
            return 'balance_bonus';
